...
int FirstDayOfMonth(int month, int year)//这个函数很好很强大！！！！！
{
	int a,b,c,d;
	a=(year-1901)/4;//这是为了看有几个闰年，所以这样多减去了一年！//
	b=(365*4+1)*a+(year-4*a-1900)*365;//前半部分是闰年之前的天数和，后半部分是最后一个
	//闰年之后的所有天数的总和，比如1912年前面有1900到1904和1908 两个闰年，1，2，3，4，
	//5，6，7，8  算完后还有1900  1909 1910  1911 1911四年  即 ：1912-4*2-1900=4 
	d=(year-1900)/400;//但是每隔100年就不算一个闰年，而隔上400年再算，所以每400年有3个不是
	//闰年，每个闰年多加了一天 ，所以减去3个d，如下！！
	c=0;
	for(int i=1;i<month;i++)
	c=c+MonthDays(i,year);//今年之前的几个月的天数。
	return((b+c-3*d+1)%7);
}
