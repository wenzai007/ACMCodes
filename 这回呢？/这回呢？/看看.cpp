...
int FirstDayOfMonth(int month, int year)
{
	int a,b,c,d;
	a=(year-1901)/4;
	b=(365*4+1)*a+(year-4*a-1900)*365;//先算前面有多少个闰年，前面的天数加上后面的 剩下的
	   //天数  就是一共的；
	d=(year-1900)/400;//其中每一百年就不算是一个闰年，但是每四百年就算一个闰年，减去3个闰年
	   //     因为每个闰年多了一天  所以这样。。。。。！！
	c=0;
	for(int i=1;i<month;i++)
	c=c+MonthDays(i,year);//今年的这个月之前的天数
	return((b+c-3*d+1)%7);//  看看每个月第一天是星期几  除以7的余数！
}
